...
class HIOKI_8450():
    ...
    def query_channel(self, channel):
        if self.instrument:
            #選擇模組中的通道是否顯示測量
            self.instrument.write(f":UNIT:STORe? {channel}")
            return self.instrument.read()
        else:
            print("No instrument to close.")

    def set_measure_mode(self, channel, mode):
        if self.instrument:
            #設定測量通道的模式 [電壓、熱電偶、濕度]
    ...
    def query_measure_mode(self, channel):
        if self.instrument:
            #設定測量通道的模式 [電壓、熱電偶、濕度]
            self.instrument.write(f":UNIT:INMOde? {channel}")
            return self.instrument.read()
        else:
            print("No instrument to close.")

    def set_rtd_type(self, channel, type):
        if self.instrument:
            #設定熱電偶線的類型[K、J、E、T、N、R、S、C]
            self.instrument.write(f":UNIT:SENSor {channel},{type}")
            print(f"set the sensor kind for the {channel} be {type}")
        else:
            print("No instrument to close.")

    def query_rtd_type(self, channel):
        if self.instrument:
            self.instrument.write(f":UNIT:SENSor? {channel}")
            return self.instrument.read()
        else:
            print("No instrument to close.")
    ...
    def query_data(self):
        if self.instrument:
            ...
            simplified_data = float(data.strip().replace('+', ''))
            query_data = round(simplified_data, 2)  # 保留小數點後兩位
            return query_data
        else:
            raise Exception("Instrument not connected. Please connect first.")
